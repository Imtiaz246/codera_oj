...
func UpdateProblem(ctx *fiber.Ctx) error {
	...
	if problem.AuthorID != user.ID {
		return ctx.Status(http.StatusForbidden).JSON(utils.NewError(fmt.Errorf("access forbidden")))
	}
...
func AddDataset(ctx *fiber.Ctx) error {
	...
	dataset, err := req.NewModelDatasetFormat(inputFile, outputFile, uint(pid), user.ID)
	if err != nil {
		return ctx.Status(http.StatusBadRequest).JSON(utils.NewError(err))
	}

	if err = models.CreateRecord(dataset); err != nil {
		return ctx.Status(http.StatusInternalServerError).JSON(utils.NewError(err))
	}

	return ctx.Status(http.StatusCreated).JSON(utils.NewSuccessResp(nil, "dataset added successfully"))
}
...
func AddProblemSolution(ctx *fiber.Ctx) error {
	...
	if err = models.GetRecordByModel(problem, problem.GetSharedUserColumnName()); err != nil {
		return ctx.Status(http.StatusForbidden).JSON(utils.NewError(err))
	}

	sharedWith := problem.FindSharedUser(user.ID)
	if sharedWith == nil || (sharedWith != nil && !sharedWith.CanAddSolution()) {
		return ctx.Status(http.StatusForbidden).JSON(utils.NewError(fmt.Errorf("forbidden")))
	}
	...
	if err = models.CreateRecord(solution); err != nil {
		return ctx.Status(http.StatusInternalServerError).JSON(utils.NewError(err))
	}

	return ctx.Status(http.StatusCreated).JSON(utils.NewSuccessResp(structs.NewSolutionResponse(solution, sharedWith)))
}
